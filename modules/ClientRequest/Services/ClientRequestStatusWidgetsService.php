...
use Carbon\Carbon;
use Modules\ClientRequest\Presenters\ClientRequestStatusWidgetsPresenter;

class ClientRequestStatusWidgetsService
{
    ...
    public function getClientRequestStatusStatistics()
    {
        $now = Carbon::now();
        ...
        return new ClientRequestStatusWidgetsPresenter(
            $currentStats['total'],
    ...
    protected function getStatistics(Carbon $date)
    {
        ...
        return [
            ...
            'pending' => (clone $query)->where('status_client_request', 'pending')->count(),
            'accepted' => (clone $query)->where('status_client_request', 'accepted')->count(),
            'rejected' => (clone $query)->where('status_client_request', 'rejected')->count(),
        ];
    }

    public function clearWidgetCache(): void
    {
        Cache::forget('client_request_status_widget_statistics-' . app()->getLocale());
    }
}
