...
final class InboxItemPresenter
{
    ...
    private static function stepFromProcess(EmployeeTaskRequest $task): ?array
    {
        ...
        if ($processStep->relationLoaded('assignedUser') && $processStep->assignedUser) {
            $namesById[(string) $processStep->assignedUser->id] = $processStep->assignedUser->name;
        }
        $missingIds = array_values(array_filter($userIds, fn ($id) => !isset($namesById[(string) $id])));
        if (!empty($missingIds)) {
            $namesById += \Modules\User\Models\User::whereIn('id', $missingIds)
                ->get()
                ->pluck('name', 'id')
        ...
        foreach ($userIds as $userId) {
            $actionTakers[] = ['user_id' => $userId, 'name' => $namesById[(string) $userId] ?? null];
        }
