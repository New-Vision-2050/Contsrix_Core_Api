...
class CreateOfficialDocumentFromLegalData
{
    /**
     * Handle the event.
     *
     * @param CompanyLegalDataCreated $event
     * @return void
     */
    public function handle(CompanyLegalDataCreated $event): void
    {
        $this->createOfficialDocument($event->legalData);
    }
    ...
    protected function createOfficialDocument(CompanyLegalData $legalData): void
    {
        $officialDocument = CompanyOfficialDocument::create([
            ...
            'start_date' => $legalData->start_date ? Carbon::parse($legalData->start_date)->format('Y-m-d') : null,
            'end_date' => $legalData->end_date ? Carbon::parse($legalData->end_date)->format('Y-m-d') : null,
            'notification_date' => $legalData->end_date ? Carbon::parse($legalData->end_date)->subDays(7)->format('Y-m-d') : null,
