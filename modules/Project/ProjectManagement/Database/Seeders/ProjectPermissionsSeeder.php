...
class ProjectPermissionsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (empty($configPermissions)) {
            Log::warning('No permissions found in project-management config');
        ...
        foreach ($configPermissions as $key => $name) {
            // Find matching permission data
            $permData = $permissionsData[$key] ?? null;

            if (!$permData) {
                Log::warning("Permission data not found for key: {$key}");
                continue;
            }
            ...
            ProjectPermission::updateOrCreate(
                ['name' => $name],
                [
                    'submodule' => $permData['submodule'],
                    'action' => $permData['action'],
                    'title' => [
                        'ar' => $permData['title_ar'],
                        'en' => $permData['title_en'],
                    ],
