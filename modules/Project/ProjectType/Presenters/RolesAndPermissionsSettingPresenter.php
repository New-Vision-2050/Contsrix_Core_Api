<?php

declare(strict_types=1);

namespace Modules\Project\ProjectType\Presenters;

use Modules\Project\ProjectType\Models\RolesAndPermissionsSetting;
use BasePackage\Shared\Presenters\AbstractPresenter;

class RolesAndPermissionsSettingPresenter extends AbstractPresenter
{
    private RolesAndPermissionsSetting $setting;

    public function __construct(RolesAndPermissionsSetting $setting)
    {
        $this->setting = $setting;
    }

    protected function present(bool $isListing = false): array
    {
        return [
            'id' => $this->setting->id,
            'project_type_id' => $this->setting->project_type_id,
            'is_enabled' => $this->setting->is_enabled,
            'created_at' => $this->setting->created_at?->toDateTimeString(),
            'updated_at' => $this->setting->updated_at?->toDateTimeString(),
        ];
    }
}
