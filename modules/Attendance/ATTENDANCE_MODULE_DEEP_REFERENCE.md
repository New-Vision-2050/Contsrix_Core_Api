...
counts as a present day and its hours accrue normally — but the report prints its
time under `بدء مهمة` / `نهاية مهمة` instead of the office columns, so a reader can
see the employee worked away from their own location.

That attribution cannot be recomputed later. Validation asks only "is this
coordinate inside any allowed circle", and the allowed list is a flat merge of
branch locations, `attendance_constraint_locations`, locations from the user's
additional constraints, and task geofences; the match discards which circle won.
The task geofence itself is transient — `EmployeeTaskTemporaryLocationProvider`
publishes it only while the task is `in_progress` and inside
`time_from + duration_hours` — so by the time a report runs, the circle that
accepted the punch no longer exists. It is therefore recorded on the row:
`attendances.clock_in_task_id` and `attendances.clock_out_task_id`, resolved in
...
finish at a task site, or the reverse, and each punch lands in its own column pair.
...
When the time becomes visible in the report differs by task kind
(`ReportDataExtractionService::taskPunchIsReportable`):

- **Project notification** (`is_project_notification = true`) — as soon as the punch
  exists.
- **A task the employee raised themselves** — only at `status = completed`.
  `EmployeeTaskLifecycleService::performEnd` is the only writer of that status and
  runs after the End-Task procedure resolves (immediately when no procedure applies
  or it auto-approves, otherwise once the admin steps finish), so this single check
  also covers "approved and every procedure step done".

While a punch is withheld its time appears in neither column pair, but the day still
reads `حضور` with its hours: presence comes from the attendance row, which is not in
question — only where to print the time is.
