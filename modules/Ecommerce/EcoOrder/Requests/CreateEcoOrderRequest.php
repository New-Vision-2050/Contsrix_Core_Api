...
class CreateEcoOrderRequest extends FormRequest
{
    public function rules(): array
    {
        return [
            ...
            'details.*.eco_product_id' => 'required|uuid',
            'details.*.shipping_method_id' => 'nullable|uuid',
            'details.*.warehouse_id' => 'nullable|uuid',
            'details.*.digital_file_after_sell' => 'nullable|string',
            'details.*.product_details' => 'nullable|array',
            'details.*.qty' => 'required|integer|min:1',
            'details.*.price' => 'required|numeric|min:0',
            'details.*.tax' => 'numeric|min:0',
            'details.*.discount' => 'numeric|min:0',
            'details.*.tax_model' => 'string|in:include,exclude',
            'details.*.delivery_status' => 'string|in:pending,confirmed,processing,out_for_delivery,delivered,returned,failed,canceled',
            'details.*.payment_status' => 'string|in:paid,unpaid,partially_paid',
            'details.*.variant' => 'nullable|string',
            'details.*.variation' => 'nullable|string',
            'details.*.discount_type' => 'string|in:amount,percentage',
            'details.*.is_stock_decreased' => 'boolean',
            'details.*.refund_request' => 'integer|min:0',
        ];
    }
    ...
    public function createCreateEcoOrderDTO(): CreateEcoOrderDTO
    {
        return new CreateEcoOrderDTO(
            ...
            payment_status: $this->get('payment_status', 'unpaid'),
            order_status: $this->get('order_status', 'pending'),
            ...
            order_amount: (float) $this->get('order_amount', 0),
            paid_amount: (float) $this->get('paid_amount', 0),
            ...
            admin_commission: (float) $this->get('admin_commission', 0),
            is_pause: $this->get('is_pause', '0'),
            cause: $this->get('cause'),
            shipping_address: $this->get('shipping_address'),
            discount_amount: (float) $this->get('discount_amount', 0),
            discount_type: $this->get('discount_type'),
            coupon_code: $this->get('coupon_code'),
            coupon_discount_bearer: $this->get('coupon_discount_bearer', 'inhouse'),
            shipping_responsibility: $this->get('shipping_responsibility'),
            shipping_method_id: $this->get('shipping_method_id'),
            shipping_cost: (float) $this->get('shipping_cost', 0),
            is_shipping_free: $this->boolean('is_shipping_free'),
            order_group_id: $this->get('order_group_id', 'def-order-group'),
            verification_code: $this->get('verification_code', '0'),
            verification_status: $this->boolean('verification_status'),
            shipping_address_data: $this->get('shipping_address_data'),
            delivery_man_id: $this->get('delivery_man_id'),
            deliveryman_charge: (float) $this->get('deliveryman_charge', 0),
            ...
            order_type: $this->get('order_type', 'default_type'),
            extra_discount: (float) $this->get('extra_discount', 0),
            extra_discount_type: $this->get('extra_discount_type'),
            refer_and_earn_discount: (float) $this->get('refer_and_earn_discount', 0),
