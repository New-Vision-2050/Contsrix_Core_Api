...
>
> **Update 2026-07-10 (rev 2)**: `ProjectNotificationService::reassignTask()` now accepts an array of user IDs via `assigned_user_ids`, replacing the notification's assigned users and creating per-user creation-workflow processes for every supplied user, identical to notification creation with multiple assignees.
...
#### 2.1.9 `ProjectNotificationRead` (table: `project_notification_reads`)

**File**: `Modules\Project\ProjectManagement\Models\ProjectNotificationRead`
...
**Fillable Fields**:
```
project_notification_id, user_id, read_at
```
...
**Purpose**: Per-user read/unread tracking for project notifications. A row exists when the user has marked the notification as read; absence means unread. The `is_read` boolean is injected into list/detail/map presenter responses for the current user so the frontend can style unread rows with a different background.
...
- `reassignTask(string $notificationId, array $targetUserIds, string $actorUserId): ProjectNotification` — Reassigns the linked `EmployeeTaskRequest` to one or more employees and resets lifecycle markers so each new assignee starts a fresh lifecycle. The supplied `$targetUserIds` replace `assigned_user_ids` (mirroring creation), `independent_progress` is enabled, the task `user_id` is switched to the first supplied user, and `confirmation_receive_date`, `location_confirmed_at`, and `end_task_status_id` are cleared. All old in-progress processes for the task are cancelled. A fresh per-user `CreateProjectNotificationTask` process is started for every supplied user via `EmployeeTaskRequestService::createIndependentProcessesForUsers()`, using the same form key and central workflow resolution as initial creation. Each user receives the notification in their inbox with the same notifications/steps as creation. If the engine auto-approves for a user (no resolvable action takers), a minimal fallback `ConfirmProjectNotificationPresence` process is seeded for that user. The pending-process guard checks by user + task, not by form, to avoid duplicate processes.
...
**Reassignment**: `ProjectNotificationService::reassignTask()` accepts an array of target user IDs, replaces `assigned_user_ids` with that list, enables `independent_progress`, switches `EmployeeTaskRequest.user_id` to the first supplied user, and clears lifecycle markers (`confirmation_receive_date`, `location_confirmed_at`, `end_task_status_id`). It cancels all old in-progress processes for the task and then starts a fresh per-user `CreateProjectNotificationTask` process for every supplied user via `EmployeeTaskRequestService::createIndependentProcessesForUsers()`, reproducing the same workflow state the notification had immediately after creation with multiple assignees. Each reassigned user may therefore see a `CreateProjectNotificationTask` process in their inbox first; `confirmReceive()` will approve it and then move into the per-user `ConfirmProjectNotificationPresence` lifecycle, exactly as happens for a task created the first time.
...
- `reassign` (POST `/projects/notifications/{id}/reassign`) accepts: `assigned_user_ids` (required, array, min 1, each UUID must exist in `users.id`). Replaces the notification's assigned users with the submitted list, enables `independent_progress`, clears lifecycle markers, cancels all old in-progress processes, resets the task status to `approved`, and creates a fresh per-user `CreateProjectNotificationTask` process for every supplied user so each can confirm receipt and start a fresh lifecycle.
