...
> **Auto-approve recording**: When a caller passes an explicit `$resolvedSetting` (lifecycle forms such as `endTask`, `startTask`, `updateProjectNotificationSiteStatus`, etc.) and the setting has no resolvable steps, `WorkflowEngine::startWorkflow()` automatically fires `WorkflowProcedureTaken`. The `RecordInternalProcedureTaken` listener records it in `internal_procedure_takens`, so the procedures timeline and available-actions ordering stay consistent without any caller-side patching.
...
| `startTask` | Employee starts the task | `WorkflowEngine::startWorkflow()` auto-fires `WorkflowProcedureTaken` when the resolved `startTask` setting has no resolvable steps; otherwise `ProcedureWorkflowService::advance()` auto-marks when `isFinal = true` |
| `endTask` | Task is ended directly (no procedure) | `WorkflowEngine::startWorkflow()` auto-fires `WorkflowProcedureTaken` when the resolved `endTask` setting has no resolvable steps; otherwise `ProcedureWorkflowService::advance()` auto-marks when `isFinal = true` |
