...
# AWS Rekognition (face verification) — optional feature, defaults to disabled.
# These are REAL AWS credentials, separate from AWS_KEY/AWS_SECRET (DigitalOcean Spaces).
FACE_RECOGNITION_ENABLED="${FACE_RECOGNITION_ENABLED:-false}"
AWS_REKOGNITION_REGION="${AWS_REKOGNITION_REGION:-us-east-1}"
FACE_MATCH_SIMILARITY_THRESHOLD="${FACE_MATCH_SIMILARITY_THRESHOLD:-80}"
FACE_LIVENESS_CONFIDENCE_THRESHOLD="${FACE_LIVENESS_CONFIDENCE_THRESHOLD:-90}"
...
TWILIO_VOICE_API_KEY_SID="${TWILIO_VOICE_API_KEY_SID}"
TWILIO_VOICE_API_KEY_SECRET="${TWILIO_VOICE_API_KEY_SECRET}"
...
PCLOUD_DISPATCH="${PCLOUD_DISPATCH:-queue}"
...
FACE_MATCH_SIMILARITY_THRESHOLD="${FACE_MATCH_SIMILARITY_THRESHOLD}"
FACE_LIVENESS_CONFIDENCE_THRESHOLD="${FACE_LIVENESS_CONFIDENCE_THRESHOLD}"

EOF

if [ "${FACE_RECOGNITION_ENABLED}" = "true" ] && { [ -z "${AWS_REKOGNITION_ACCESS_KEY_ID}" ] || [ -z "${AWS_REKOGNITION_SECRET_ACCESS_KEY}" ]; }; then
  echo "WARNING: FACE_RECOGNITION_ENABLED=true but AWS_REKOGNITION_ACCESS_KEY_ID / AWS_REKOGNITION_SECRET_ACCESS_KEY are empty."
  echo "Add GitHub Actions secrets AWS_REKOGNITION_ACCESS_KEY_ID and AWS_REKOGNITION_SECRET_ACCESS_KEY, then redeploy."
fi
