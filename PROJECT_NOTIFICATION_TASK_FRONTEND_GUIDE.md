...
> **Version**: 1.1 (aligned with backend plan v1.3; corrected API response envelope, schema name, permission keys)  
...
- The tab is **schema-driven**; the top-level tab **الصيانة والطوارئ** appears only when the backend returns schema `12` for the project's type. **الإشعارات** is its first sub-view (المخالفات / التقارير / المؤشرات are placeholders for v1).
...
|----------|--------|---------|
| `/api/v1/project-types/{id}/schemas` | GET | Determine if the Notifications tab should be shown. |
...
function canShowSchema(schemaId) {
  if (schemaId === MAINTENANCE_SCHEMA_ID) {
    return hasPermission('PROJECT_NOTIFICATION_LIST');
  }
  return true;
}
</script>
...
- [ ] Tab **الصيانة والطوارئ** appears only when schema `12` is in project type schemas (`payload[]`).
- [ ] Tab hidden when user lacks `PROJECT_NOTIFICATION_LIST`.
