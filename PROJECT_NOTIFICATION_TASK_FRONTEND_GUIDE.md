...
| `/api/v1/projects/notifications/filters` | GET | Mobile: filter metadata — same format as `GET /employee-tasks/filters` (statuses with `title_ar`/`title_en`, projects with `key`/`title`, duration in minutes). |
...
- `GET /projects/notifications/filters` — filter metadata for the mobile filter UI. **Same response shape as `GET /employee-tasks/filters`**: `statuses` (key, title_ar, title_en, count), `projects` (key, title, count), `duration` (key, title_ar, title_en, min_minutes, max_minutes). Reuse the same filter UI component as employee tasks.
...
### Complete Lifecycle (Mobile)

```
Admin creates notification (dashboard wizard)
    │
    ▼
Notification status = pending
    │
    ▼  (workflow auto-approve or admin approves)
Notification status = approved
    │
    ▼  ← appears in GET /my-inbox
Engineer opens inbox, taps "تأكيد استلام"
    │
    ▼  POST /{id}/confirm-receive
    │   form = confirmProjectNotificationPresence
    │   Task status → in_progress
    │   Notification status → in_progress (auto-synced by observer)
    │
    ▼  ← disappears from /my-inbox, appears in GET /my-tasks
Engineer can optionally:
    │
    ├─ GET /{id}/available-actions → see unlocked procedures
    │
    ├─ POST /{id}/take-action (form = updateProjectNotificationTask)
    │   Mid-task update, task stays in_progress
    │
    └─ POST /{id}/end (form = endProjectNotificationTask)
        Task status → completed
        Notification status → completed (auto-synced by observer)
```

### Using `available-actions`

The `GET /projects/notifications/{id}/available-actions` endpoint returns the same shape as `GET /employee-tasks/{id}/available-actions`:
...
```json
{
  ...
  "payload": [
    {
      "id": "uuid (procedure_setting_id)",
      "name": "تأكيد استلام",
      "form": { "key": "confirmProjectNotificationPresence", "label_ar": "تأكيد استلام" },
      "conditions": [],
      "appears_after_ids": [],
      "appears_before_ids": [],
      "sort_order": 500
...
1. Call `available-actions` when the engineer opens a notification detail.
2. Render a button for each available action, labeled with `form.label_ar`.
3. When the engineer taps a button, call the appropriate endpoint:
   - `confirmProjectNotificationPresence` → `POST /{id}/confirm-receive`
   - `updateProjectNotificationTask` → `POST /{id}/take-action`
   - `endProjectNotificationTask` → `POST /{id}/end`
4. Pass the `id` from the available-action item as `internal_procedure_setting_id` in the request body.
...
- [ ] `GET /filters` response matches `GET /employee-tasks/filters` shape (statuses with `title_ar`/`title_en`, projects with `key`/`title`, duration in `min_minutes`/`max_minutes`).
- [ ] `GET /{id}/available-actions` returns procedure list with `form.key` and `form.label_ar`; mobile renders buttons per available action.
- [ ] `POST /{id}/take-action` with `internal_procedure_setting_id` from available-actions records the update procedure.
- [ ] `POST /{id}/end` completes the task; notification status auto-syncs to `completed`.
- [ ] Notification status auto-syncs from the linked task (no manual refresh needed after confirm-receive or end).
